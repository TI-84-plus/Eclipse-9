...
class TileMap : public sf::Drawable{
	...
	void TilePosition(int TileX, int TileY, int unsigned_x, int unsigned_y, int chunk_x, int chunk_y) 
	{
		...
		float unsigned_chunkX = float((chunk_x+(chunkSize)));
		float unsigned_chunkY = float((chunk_y+(chunkSize)));
		sf::Vector2f tile_pos = {float(unsigned_x), float(unsigned_y)};
		sf::Vector2f chunk_pos = {unsigned_chunkX, unsigned_chunkY};

		vertexArray.append(sf::Vertex((((sf::Vector2f(0.0f, 0.0f) + tile_pos) * (unsigned_chunkX * (chunkSize/2)))) * tileWorldDimension, sf::Vector2f(tileTextureDimension * tile.X, tileTextureDimension * tile.Y)));
		vertexArray.append(sf::Vertex((((sf::Vector2f(1.0f, 0.0f) + tile_pos) * (unsigned_chunkX * (chunkSize/2)))) * tileWorldDimension, sf::Vector2f(tileTextureDimension * tile.X, tileTextureDimension * tile.Y)));
		vertexArray.append(sf::Vertex((((sf::Vector2f(1.0f, 1.0f) + tile_pos) * (unsigned_chunkX * (chunkSize/2)))) * tileWorldDimension, sf::Vector2f(tileTextureDimension * tile.X, tileTextureDimension * tile.Y)));
		vertexArray.append(sf::Vertex((((sf::Vector2f(0.0f, 1.0f) + tile_pos) * (unsigned_chunkX * (chunkSize/2)))) * tileWorldDimension, sf::Vector2f(tileTextureDimension * tile.X, tileTextureDimension * tile.Y)));

		// std::cout<<(float)unsigned_x+(chunk_x + chunkSize/2)<<std::endl;
	}
	...
	void ChunkGen(int chunk_x, int chunk_y)
    {
		...
		for(int y = -(height); y < height; y++) 
		{
			for(int x = -(width); x < width; x++) 
			{
				...
				int unsigned_y = (y+(height));
				int unsigned_x = (x+(width));
				...
				{
					TilePosition(3, 1, unsigned_x, unsigned_y, chunk_x, chunk_y);
				}
